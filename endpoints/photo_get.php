...
//essa funcao serve para buscarmos as informacoes
...
function api_photo_get($request) {
  ...
  //se nao existir ou se ele estiver vazio
  ...
  update_post_meta($post_id, 'acessos', $photo['acessos']);//atualizando o n de acessos
...
//segunda parte da api quando nao ha referencia a uma foto especifica

function api_photos_get($request) {
  $_total = sanitize_text_field($request['_total']) ?: 6;
  $_page = sanitize_text_field($request['_page']) ?: 1;
  $_user = sanitize_text_field($request['_user']) ?: 0; //se nao for definido, retornamos 0
